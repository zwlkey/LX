...
//		}
	...
	//实时XYZ-YAW期望速度(实时控制帧)
		rt_tar.st_data.yaw_dps = Yaw_Dps_Out;  //航向转动角速度，度每秒，逆时针为正
		rt_tar.st_data.vel_x = Vel_X_Out;    //头向速度，厘米每秒
		rt_tar.st_data.vel_y = Vel_Y_Out;    //左向速度，厘米每秒
		rt_tar.st_data.vel_z = Vel_Z_Out;	   //天向速度，厘米每秒
		//########################################################################
		//=====
	...
	else //无遥控信号
	{
		...
		if (fc_sta.unlock_sta != 0)
		{
			...
			if (fail_safe_change_mod == 0)
			{
				...
				//fail_safe_change_mod = LX_Change_Mode(2);
			}
