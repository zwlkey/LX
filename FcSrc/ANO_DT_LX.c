...
void ANO_DT_Init(void)
{
	...
	dt.fun[0xF1].fre_ms = 150;	  //触发发送的周期100ms
	dt.fun[0xF1].time_cnt_ms = 2; //设置初始相位，单位1ms
...
extern _PID_val_st my_yaw_val_2[2] ;
extern length ks;
extern float Yaw_Dot_out;
extern float len_out_dot;
extern _tracking_ctrl_st tracking1; 
extern _tracking_ctrl_st tracking2; 
extern _ano_of_st ano_of;  //光流高度
extern _fc_att_un fc_att;  //
extern struct _rol_pit_ mcu_rx; 
extern float Yaw_Dps_Out;  //闭环输出
extern float Vel_X_Out;
extern float Vel_Y_Out;
extern float Vel_Z_Out;
extern _loc_ctrl_st ur_loc_c;    //下openmv找点
extern struct _MV_ MV;
extern struct _rol_pit_ mcu_rx; 
extern struct MYMODE my_flight; 

extern _tracking_ctrl_st tracking1;   //巡线时的流程控制 
extern _tracking_ctrl_st tracking2;
extern _tracking_ctrl_st tracking3;   //巡线时的流程控制 
extern _tracking_ctrl_st tracking4;
extern _tracking_ctrl_st tracking5;   //巡线时的流程控制 
extern _tracking_ctrl_st tracking6;
...
static void Add_Send_Data(u8 frame_num, u8 *_cnt, u8 send_buffer[])
{
	...
	switch (frame_num)
	{
			case 0xF1:    //用户自定义波形 不能单独发送,会打乱原本发送顺序
		{
			   s16 _temp;		
			
//				_temp =(s16)  ano_of.of_alt_cm; //  光流输出对地高度
//				send_buffer[(*_cnt)++]=BYTE0(_temp);
//				send_buffer[(*_cnt)++]=BYTE1(_temp);
//			
			
				_temp =(s16) tracking3.step ; //1
				send_buffer[(*_cnt)++]=BYTE0(_temp);
				send_buffer[(*_cnt)++]=BYTE1(_temp);
			
				_temp =(s16) tracking3.time ; //2
				send_buffer[(*_cnt)++]=BYTE0(_temp);
				send_buffer[(*_cnt)++]=BYTE1(_temp);
			
				_temp =(s16)  my_flight.mode; //3
				send_buffer[(*_cnt)++]=BYTE0(_temp);
				send_buffer[(*_cnt)++]=BYTE1(_temp);
			
				_temp =(s16)  dot.flag; //4
				send_buffer[(*_cnt)++]=BYTE0(_temp);
				send_buffer[(*_cnt)++]=BYTE1(_temp);
				...
				_temp =(s16)  Vel_X_Out; //9
				send_buffer[(*_cnt)++]=BYTE0(_temp);
				send_buffer[(*_cnt)++]=BYTE1(_temp);
				
				_temp =(s16)  Vel_Y_Out; //10
				send_buffer[(*_cnt)++]=BYTE0(_temp);
				send_buffer[(*_cnt)++]=BYTE1(_temp);

				_temp =(s16)  my_flight.mode; //  光流输出对地高度
				send_buffer[(*_cnt)++]=BYTE0(_temp);
				send_buffer[(*_cnt)++]=BYTE1(_temp);
				
				_temp =(s16)    dot.flag; //5
				send_buffer[(*_cnt)++]=BYTE0(_temp);
				send_buffer[(*_cnt)++]=BYTE1(_temp);

					_temp =(s16)  mcu_rx.pit; //6
				send_buffer[(*_cnt)++]=BYTE0(_temp);
				send_buffer[(*_cnt)++]=BYTE1(_temp);	
