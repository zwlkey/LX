...
//上升高度
u8 Up_Move(u16 distance_cm,u16 velocity_cmps)  //距离  速度
{
	  if (dt.wait_ck == 0) //没有其他等待校验的CMD时才发送本CMD
	{
		//按协议发送指令
...
//下降高度
u8 Down_move(u16 distance_cm,u16 velocity_cmps) 
{
	  if (dt.wait_ck == 0) //没有其他等待校验的CMD时才发送本CMD
	{
		//按协议发送指令
...
//左旋角度
u8 Turn_Lift(u16 distance_deg,u16 velocity_degs)  //角度  速度
{
		  if (dt.wait_ck == 0) //没有其他等待校验的CMD时才发送本CMD
	{
		//按协议发送指令
		...
		dt.cmd_send.CMD[1] = 0X07;
		//
		dt.cmd_send.CMD[2] = BYTE0(distance_deg);
		dt.cmd_send.CMD[3] = BYTE1(distance_deg);
		dt.cmd_send.CMD[4] = BYTE0(velocity_degs);
		dt.cmd_send.CMD[5] = BYTE1(velocity_degs);
		//
		CMD_Send(0xff, &dt.cmd_send);
		return 1;
	}
...
//右旋角度
u8 Turn_Right(u16 distance_deg,u16 velocity_degs)   //角度  速度
{
		  if (dt.wait_ck == 0) //没有其他等待校验的CMD时才发送本CMD
	{
		//按协议发送指令
